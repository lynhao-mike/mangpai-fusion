...
> **状态口径**：本文以“状态矩阵”区分已落地 / 规划中 / 延后 / 废弃；实现状态以矩阵为准，避免把 v1.4 规划项误读为全部已发布。
...
| **V5** | 🟡 规划中 | Schema 设计已锁定，尚未完成 dataclass / 历史 fixture round-trip | v1.4 W2：Track 1 schema 升级 |
| **V6** | 🟡 规划中 | 字段设计已锁定，尚未完成财富分级框架切换 | v1.4 W2：与 V5 同步实现 |
| **V7** | 🟡 规划中 | 模板需求已锁定，需等待 V5/V6 数据字段稳定 | v1.4 W3：模板与输出 UX 收口 |
| **V8** | 🟡 规划中 | 工具需求已锁定，现阶段可用 output_linter 做手工替代 | v1.4 W3：实现 `cross_domain_consistency_check.py --backfill` |
...
| `feat/v1.4-track-1-schema` | Track 1 schema 升级（V5/V6） | 🟡 下一步，v1.4 收口优先级 1 |
| `feat/v1.4-track-4-coupling` | Track 4 跨域耦合 + 模板（V7/V8） | 🟡 下一步，v1.4 收口优先级 2 |
| `feat/v1.4-track-5-marriage` | Track 5 婚姻模型 | ⏸️ 延后到 v1.5，需 N_eff 达阈值后再统计建模 |

---

**本文件由 v1.4 启动 session 于 2026-05-26 创建。**
**当前口径：v1.4 Track 2+3 已落地；V5-V8 仍处规划/收口阶段，后续按状态矩阵推进。**
