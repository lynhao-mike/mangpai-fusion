...
3. 若用户首次提交新命盘并要求“分析八字 / 看这个八字 / 分析这个八字”，应先判断是否需要形成正式报告：
   - 若用户同时要求“生成报告 / 形成报告 / 出报告 / 输出报告”，或后续追问要求报告，则本次首次分析必须升级为报告流程，执行 5.2 的双侧归档。
   - 若仅为临时口头分析、不要求报告，可先答复分析；但一旦后续要求报告，必须把首次分析内容补入同名 case 与 report，不得只在会话中保留。
4. 运行入口校验：
...
> **最高硬性约束**：当用户要求“生成报告 / 形成报告 / 出报告 / 输出报告”时，必须同时完成 [`reports/`](reports/) 与 [`cases/`](cases/) 两侧归档；只生成报告文件而不建立对应 case 目录与文件，视为未完成任务。
>
> **新案例首次分析约束**：新案例的首次正式分析若需要输出报告，必须在同一轮任务内生成正式报告文件，并同步建立/更新同名 case 目录；不得先只给会话分析、事后遗漏报告或 case 归档。
...
8. 若是先会话分析、后要求报告，需把会话首次分析与后续反馈校准一并归档到 `analysis.md` / `feedback.md` / `statement_index.json`，再输出 report。
