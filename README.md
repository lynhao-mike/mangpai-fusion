...
新命盘首次分析时，若用户要求“分析八字 / 看这个八字 / 断这个八字 / 生成报告 / 形成报告 / 出报告 / 输出报告”，默认生成《命理师内容报告（统一版）》，必须把推演过程、裁决结论、三派依据、应期判断与风险提示融合进一份报告，并同步建立同名 case 结构化归档。自 V2 起取消“双侧报告模式”，不再分别生成 `analysis.md` 与 `analyst-report.md` 两份可读报告。除非用户明确要求“生成用户报告 / 客户报告 / 命主可读报告 / 对外报告”，否则不得生成用户报告。统一报告必须覆盖学业 / 事业 / 财富 / 婚姻 / 健康 / 性格等主要事项；每个事项必须给出 15 层判断，格式至少包含“判断结果｜证据链｜置信度｜应期”。

### 生成报告

生成报告必须同时完成单报告归档与 case 结构化归档：

- `reports/C-YYYY-NNN-{乾/坤}-{四柱}-content-report.md`
- `cases/C-YYYY-NNN-{乾/坤}-{四柱}/input.md`
- `cases/C-YYYY-NNN-{乾/坤}-{四柱}/feedback.md`
- `cases/C-YYYY-NNN-{乾/坤}-{四柱}/statement_index.json`

若需要保留内部推理审计，可写入 case 目录下的结构化内部文件；不得再生成第二份可读 `analysis.md` 报告。
