...
│  正式主流程 = 五派独立命题 + 结构图归并 + 三段式仲裁。       │
│  主要事件 = 先入预测账本，再进入报告与反馈学习。             │
└─────────────────────────────────────────────────────────────┘
```

当前目标模型不再把旧 D1-D4 串行维度引擎当作方法论主流程。正式方向见 [`10-ziping-fusion-v5.md`](10-ziping-fusion-v5.md) 与 [`../../plans/geju-bazi-analysis-redesign.md`](../../plans/geju-bazi-analysis-redesign.md)：同一命盘先由子平类、滴天髓类、高德臣、段建业、杨清娟五派 runner 独立产生命题，再进入结构图、三段式仲裁、主要事件预测账本、报告与反馈闭环。

旧 D1-D4 的定位改为**盲派综合 runner 内部素材**，不是全局编排权：

| 旧维度 | 原派别能力 | 现在定位 | 引擎 |
|---|---|---|---|
| **D1 能量** | 段派做功、体用、势党 | 三盲派事件落地素材 | `energy_evaluator` |
| **D2 画面** | 杨派画像、五步、生活取象 | 三盲派细节取象素材 | `picture_matcher` |
| **D3 时间** | 任系应期、触发器、三层门 | timing enhancer | `yingqi_gate` |
| **D4 旁证** | 高派神煞、健康、灾厄补强 | 结构旁证与事件反证素材 | `pangzheng_engine` |

删除“D1 是全局底盘、D2 必须消费 D1、D3 必须消费 D1+D2”的主流程叙事。需要串行消费时，只允许发生在盲派综合 runner 内部，不得重新冒充全局主流程。
...
当前运行事实：[`application/pipeline_runner.py`](../application/pipeline_runner.py) 仍是已落地兼容入口，负责旧 D1-D4 编排、报告渲染前数据产出与既有反馈链路稳定性。它是 staged fallback / 对照验证入口，不是新的长期方法论主流程。

迁移红线：五派并行主流程未跑出“六大领域 × 十五层判断 × 三段裁判 × 主要事件预测账本”的已归档案例证明物前，不切换正式入口；证明通过后，再把旧 W1-W4 删除或降为回归对照命令。
