...
生成报告时必须**同时**完成单报告归档与 case 结构化归档：

- `reports/C-YYYY-NNN-{乾/坤}-{四柱}-content-report.md`：《命理师内容报告（统一版）》正式归档。
- `cases/C-YYYY-NNN-{乾/坤}-{四柱}/`：同名 case 目录，至少包含 `input.md`、`feedback.md`、`statement_index.json`。

新案例首次分析若用户要求“分析八字 / 看这个八字 / 断这个八字 / 生成报告 / 形成报告 / 出报告 / 输出报告”，默认生成《命理师内容报告（统一版）》，不得先只输出会话分析；必须在同一轮任务内生成统一报告并建立同名 case。若用户明确要求临时口头分析或不归档，必须标记“未归档、不可进入反馈闭环”；后续一旦要求报告，必须将首次会话分析与后续反馈校准融合进统一报告与 case 结构化文件。只有当用户明确要求“生成用户报告 / 客户报告 / 命主可读报告 / 对外报告”时，才允许额外生成用户报告。统一报告必须覆盖学业 / 事业 / 财富 / 婚姻 / 健康 / 性格等主要事项；每个事项必须给出 15 层判断，格式至少包含“判断结果｜证据链｜置信度｜应期”。

自 V2 起取消“双侧报告模式”：不得再把同一案例拆成 `analysis.md` 与 `analyst-report.md` 两份可读报告；内部 `statement_index.json` 继续承接反馈校准和断语追踪。
