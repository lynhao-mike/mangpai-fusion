"""Promote approved first five Wenzhen staging candidates to formal case dirs.

This script is intentionally narrow-scoped for the approved first-5 promotion
batch. It reads the existing dry-run plan and staging manifest, then creates the
required formal case files without modifying source drafts or staging artifacts.
"""
...
PLAN_JSON = REPO_ROOT / "cases" / "raw_feedback" / "parsed" / "wenzhen_repan_first5_promotion_plan.json"
...
def main() -> int:
    ...
    staging_by_raw_id = _load_staging()
    created: list[str] = []
    for item in plan_payload["plans"]:
        raw_id = item["raw_id"]
        staging = staging_by_raw_id[raw_id]
        draft = _load_draft(REPO_ROOT / item["draft_path"])
        case_dir = REPO_ROOT / item["case_dir"]
        case_dir.mkdir(parents=True, exist_ok=True)
        (case_dir / "input.md").write_text(_render_input(item, staging, draft), encoding="utf-8")
        (case_dir / "analysis.md").write_text(_render_analysis(item, staging, draft), encoding="utf-8")
        (case_dir / "feedback.md").write_text(_render_feedback(item, staging, draft), encoding="utf-8")
        (case_dir / "statement_index.json").write_text(
            json.dumps(_statement_index(item, staging, draft), ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
...
def _load_staging() -> dict[str, dict[str, Any]]:
...
def _render_input(item: dict[str, Any], staging: dict[str, Any], draft: dict[str, Any]) -> str:
    bazi = staging["bazi"]
    ...
    return (
        ...
        f"> 首批问真 staging 正式转案；raw_id：`{item['raw_id']}`。\n\n"
        f"**立案日期**：{TODAY}  \n"
        f"**命主代号**：{item['raw_id']}  \n"
...
def _render_analysis(item: dict[str, Any], staging: dict[str, Any], draft: dict[str, Any]) -> str:
    return (
...
def _render_feedback(item: dict[str, Any], staging: dict[str, Any], draft: dict[str, Any]) -> str:
    lines = [
        f"# Case {item['case_id']} · 命主反馈",
        "",
        f"> 首批问真 staging 转案；本文件先迁移原 known_facts，后续报告断语反馈再补 `[S-...] [y/n/?/skip]`。",
        "",
        f"**反馈日期**：{TODAY}  ",
        "**采集方式**：问真真实反馈样本迁移  ",
        "**关联报告**：待生成  ",
...
def _statement_index(item: dict[str, Any], staging: dict[str, Any], draft: dict[str, Any]) -> dict[str, Any]:
    statements = []
