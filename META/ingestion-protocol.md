...
|---|---|---|
| S1 抽取 | 人工 + LLM 协助 | 半自动 |
| S2 归一 | `tools/normalize_extracted.py` | 自动（需人工校核） |
| S3 打分 | `tools/score_initial.py` | 自动 |
| S4 对照 | `tools/cross_map.py` | 自动（语义相似度匹配） |
| S5 入库 | git + 手动 review | 半自动 |
| 晋级/退役 | `tools/calibrate.py` | 自动（基于反馈） |

工具尚未实现的，先人工执行流程并记录于 `META/rule-changelog.md`。
