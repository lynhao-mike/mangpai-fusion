...
| rule_id | 派 | hits 旧→新 | misses 旧→新 | conf ★ 旧→新 | status 旧→新 | verdict |
|---|---|---|---|---|---|---|
| M2-Y-120 | yang | — | — | ★3→★2 | confirmed→confirmed | miss（无印透干=非体制，命主实为事业编） |
| M2-Y-035 | yang | — | — | ★3→★3 | confirmed→confirmed | hit（财富7等第2等=劳碌，年入7-8万吻合）|
| M2-Y-064 | yang | — | — | ★3→★4 | confirmed→confirmed | hit（婚期6条件=婚路迂回，二婚铁验）|
| M2-Y-070 | yang | — | — | ★3→★3 | confirmed→confirmed | hit（比劫制旺财=三本，实际三本命中下界）|
| M3-R-031 | ren | 5→6 | 3→3 | ★3→★3 | flagged_for_review→flagged_for_review | hit（巳酉丑三合金局=2013离婚铁应期）|
| G-LF-天医 | gao | — | — | ★3→★4 | candidate→candidate | hit（天医=卫健局行业缘铁命中）|
| G-LF-华盖 | gao | — | — | ★3→★4 | candidate→candidate | hit（华盖=自幼独处，命主确认）|
| G-LF-羊刃性格 | gao | — | — | ★3→★4 | candidate→candidate | hit（羊刃=倔强固执，命主确认）|
| G-LF-血刃外伤 | gao | — | — | ★3→★2 | candidate→candidate | miss（无外伤无手术，但有甲亢慢性病）|

### 新增候选规律（candidate）

- `CAND-C018-001`：坤命双酉自刑 + 金旺无泄 → 甲状腺/内分泌慢性病风险
  - 来源：C-2026-018 甲亢反馈
  - 起始置信度：★★ (55%)
  - 升级路径：再有 1 例酉自刑+甲状腺类疾病命主复现 → confirmed

- `CAND-C018-002`：天医神煞在行业判定中应获首选权重（当天医与格局判定方向冲突时，天医优先）
  - 来源：C-2026-018 卫健局反馈（天医铁应 vs 格局判民营失验）
  - 起始置信度：★★★ (68%)
  - 升级路径：再有 2 例天医对应实际行业的命主复现 → confirmed

- `CAND-C018-003`：印藏有根（不透干）时，事业编/准体制路径不应排除
  - 来源：C-2026-018（印癸藏丑有根，命主实为事业编）
  - 修正对象：M2-Y-120 "无印透干=非体制"边界条件
  - 起始置信度：★★★ (65%)
  - 升级路径：再有 1 例印藏有根+事业编命主复现 → 调整 M2-Y-120 适用范围

### 降级观察

| rule_id | 原 status | 原因 |
|---|---|---|
| M2-Y-120 | confirmed | 本案失验 + C-2026-001 事业编未失验 → 保留 confirmed，但加注边界条件（CAND-C018-003） |

### Cross-School Scan
- 未触发（case_count=12，下一次在 case_count=20 时触发）

### 关键发现

1. **高派神煞 > 段杨派格局判定**（在行业/性格域）：天医/华盖/羊刃三条全中，段杨派行业判+收入判双失验
2. **酉酉自刑 = 二婚铁律**：本案为仓库首例"酉酉自刑坤命"验证→先婚后离再婚
3. **巳酉丑三合金局 = 婚变应期**：2013癸巳离婚 = 精确年份命中
4. **丁酉年 = 酉伏吟+食神透 = 再婚年**：2017丁酉再婚 = 精确年份命中
5. **金旺克木 → 甲亢**：双酉自刑+金旺无泄 → 甲状腺疾病（候选新规律）

### 决策 E 进度
- 累计反馈样本：**12 / 30**（Beta 切换阈值），还差 18 案。
